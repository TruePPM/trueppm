...
115. **Every settings page with a save-bar form follows the dirty/save/discard contract.** Two paths:
     ...
     Pages with row-level mutations and no save-bar (e.g. `WorkspaceMembersPage`, `WorkspaceWebhooksPage`, `WorkspaceIntegrationsPage`, `WorkspaceGroupsPage`, `ProjectAccessPage`, `ProgramAccessPage`, `ProgramProjectsPage`) and danger-zone pages with typed-confirm flows (`WorkspaceDangerPage`, `ProjectArchivePage`, `ProgramArchivePage`) are exempt — they don't have a save-bar UI surface.
