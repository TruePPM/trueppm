...
## Entitlement-aware nav rows + upsell surfaces (Issue #1173, ADR-0029/0030)

178. **An Enterprise-gated *nav row* shows a working in-app upsell route + a non-interactive EE `<span>` badge; an Enterprise-gated *action/marketing surface* uses the focusable `EnterpriseBadge` link (the rule-121 split).** Rule 121's "the badge IS the link" holds for action buttons, matrix cells, and marketing CTAs — surfaces where the badge can be its own focusable `<a>`. But a **sidebar/nav row is already a link** (`NavLink`), and nesting the `EnterpriseBadge` anchor inside it is invalid HTML and a focus-order trap. So a community-edition Enterprise nav row (the `/portfolio-upsell` row, #1173) renders the EE indicator as an `aria-hidden` **`<span>`** using the rule-121 *visual* recipe (`bg-brand-primary/10 text-brand-primary text-[10px] font-semibold uppercase tracking-wide`, no `href`, no ring), and the **row itself** routes in-app to a designed upsell page (never `/portfolio` — that path is owned by the Enterprise `routes` slot, ADR-0029; use a distinct `/<feature>-upsell` route). The row's `aria-label` carries the composite name ("… — available in TruePPM Enterprise") so the SR never reads "label E E". The upsell page (`features/portfolio/PortfolioUpsellPage.tsx`) is static marketing content only — it queries no Enterprise data (Apache-2.0 boundary) — and carries the real external `trueppm.com/enterprise` CTA + the focusable `EnterpriseBadge`. It **redirects** (`<Navigate replace>`) to the real route under the enterprise edition, gated behind `useEdition().isLoading` to avoid a community-default flash (the hook defaults to `'community'` while loading). A community-edition Enterprise nav row must never be fully hidden (reads as broken OSS) and must never be a dead control (#669); it always leads somewhere that explains the gate. The row stays `showFull`-gated so the collapsed rail is unchanged.
