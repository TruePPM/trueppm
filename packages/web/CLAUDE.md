...
428. **A persistent mode-indicator bar that never changes during a session carries no `aria-live` — use `<aside aria-label="…">` for findability instead, keep responsive copy in ONE text node, and render nothing while the mode is unresolved.** → [rule](../../docs/design/invariants/428-a-persistent-mode-indicator-that-never-changes.md)
429. **A permanently-mounted live region must stay in the accessibility tree while it is empty — `empty:sr-only`, never `empty:hidden`; and responsive copy hidden with `hidden md:inline` is hidden from assistive technology too, so a sentence that must be complete at every width uses `sr-only md:not-sr-only`.** → [rule](../../docs/design/invariants/429-a-permanently-mounted-live-region-must-stay-in-the-tree.md)
...
424. **A deployment-mode refusal (`403` + `code: "demo_read_only"`) is matched on status AND code together and routed to the mode's own affordance — never to a generic error component, never to the normal preview-rejection path.** → [rule](../../docs/design/invariants/424-a-deployment-mode-refusal-is-matched-on-status-and-code.md)
425. **A client preview that must outlive its refused commit is held in a store ABOVE the query cache, never written into it with `setQueryData` — the cache stays truthful to the server.** → [rule](../../docs/design/invariants/425-a-preview-that-must-outlive-its-refused-commit-lives.md)
426. **A request-config flag that suppresses a global toast or affordance is destructured out of the request payload before the body is built — and only passed as a config arg when actually set.** → [rule](../../docs/design/invariants/426-a-request-config-flag-that-suppresses-a-global.md)
427. **A pre-login deployment signal rides the existing `GET /api/v1/edition/` on the shared `['edition']` key through the one `editionQueryOptions` — never `apiClient`, never a new discovery endpoint, never a build-time global.** → [rule](../../docs/design/invariants/427-a-pre-login-deployment-signal-rides-the-existing.md)
430. **A deployment-mode gate is all-or-nothing across a surface: every mutating control derives from one root predicate, none may reach the generic red refusal path, and the explanation is a VISIBLE sibling — never a `title` on a `disabled` element.** → [rule](../../docs/design/invariants/430-a-deployment-mode-gate-is-all-or-nothing-across-a-surface.md)
431. **A stored preference is only a preference when the login is one person's — on a shared credential (the read-only demo, a kiosk) a `…<userId>.<projectId>` key returns the LAST visitor's choice, so gate the read AND the write on the deployment and let the session own it.** → [rule](../../docs/design/invariants/431-a-preference-read-from-storage-must-be-gated-on-whether.md)
