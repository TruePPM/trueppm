...
137. **Focus-ring form in the Schedule view: `focus-visible:` wins, with the rule-4 dark-mode override.** The Schedule view (`features/schedule/`) uses `focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-1 focus-visible:ring-offset-neutral-surface` per rules 46/126 — this overrides the generic ux-review `focus:` preference for standalone controls. Because the Schedule view adapts to dark mode (rule 40) and `brand-primary` is 2.81:1 on the dark surface (WCAG 1.4.11 fail), the **`dark:focus-visible:ring-semantic-on-track` override (rule 4) is mandatory, not optional**, on every focusable Schedule control on a neutral surface — including toolbar controls like ZoomControl. Steppers inside a bordered `role="group"` may use `ring-inset` without an offset, but still carry the dark override.
