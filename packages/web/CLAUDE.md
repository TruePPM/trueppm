...
124. **The settings context pill is an entity switcher, and its chevron means "switchable".** `SettingsShell` renders `SettingsContextSwitcher` (the program/project identity row) as a `role="menu"` trigger whenever it receives `contextOptions` with **≥ 2** entries, letting the user jump straight to another program's / project's settings — the option `to` preserves the current sub-page (`…/test/settings/cadence` → `…/test2/settings/cadence`), and selection routes through the shell's `guardedNavigate` so a dirty form still hits `ConfirmDiscardDialog`. With **0–1** options it is a static identity row with **no chevron** — never render the chevron when no switch is possible (the pre-#776 always-on chevron was a dead affordance). Menu rows are `role="menuitemradio"` with `aria-checked` on the current entity and full keyboard nav (arrows/Home/End/Enter/Esc, focus returns to the trigger). The per-row health dot is `aria-hidden` (rule 6), so health is also carried in each row's and the trigger's `aria-label` (e.g. `"test2, critical"`). Workspace scope passes no `contextOptions` (single workspace) → static pill. **At `>= 8` options the popover switches from the `menu` pattern to a `combobox` search input + `role="listbox"` of `option` rows** (type-to-filter, case-insensitive substring on name; focus stays in the input, highlight via `aria-activedescendant`; two-stage Escape clears the query then closes; empty result → a `role="status"` "No …s match" row). The two ARIA patterns are chosen by `options.length` and never coexist — do not merge them into a hybrid. The search box gets its focus ring via `focus-within` (it receives programmatic focus on open, where `focus-visible` is unreliable). Covered by `SettingsContextSwitcher.test.tsx`, `SettingsShell.test.tsx`, and `e2e/program-general-settings.spec.ts`.
