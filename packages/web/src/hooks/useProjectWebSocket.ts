 ...
 * else-if chain (#2081). The registrations are grouped into small module-level
 * `register*Handlers(on, deps)` functions — each closes over nothing, taking the
 * queryClient / refs / store setters it needs through the `deps` bag — so the
 * connection effect stays a lifecycle-only closure instead of a 130+-complexity
 * wall of inline handlers (#2081). Grouped event types (the dependency_*, sprint_*,
 * … families that share one invalidation) register the same handler under every
 * member key.
 *
...
type OnFn = (types: string | readonly string[], handler: WsHandler) => void;

// Store slices whose setters the handlers dispatch into. Deriving the setter
// types from the live store state keeps them exact without hand-typing (and
// with no `any`): a store signature change surfaces here at compile time.
type SchedulerStoreState = ReturnType<typeof useSchedulerStore.getState>;
type TaskRunStoreState = ReturnType<typeof useTaskRunStore.getState>;
type PresenceStoreState = ReturnType<typeof usePresenceStore.getState>;

/**
 * Everything an event handler needs, threaded explicitly so the `register*`
 * functions can live at module scope (out of the connection effect) and close
 * over nothing but their arguments. `queryClient` and the store setters are
 * stable across the effect's lifetime; the refs give handlers the current
 * project id / seq cursor / seen-versions map without recapturing them.
 */
interface WsHandlerDeps {
  queryClient: QueryClient;
  projectIdRef: MutableRefObject<string | null | undefined>;
  seenTaskVersionsRef: MutableRefObject<Map<string, number>>;
  lastSeqRef: MutableRefObject<number>;
  scheduleInvalidate: ScheduleInvalidateFn;
  setRecalculating: SchedulerStoreState['setRecalculating'];
  setCpmError: SchedulerStoreState['setCpmError'];
  setCpmComplete: SchedulerStoreState['setCpmComplete'];
  addRun: TaskRunStoreState['addRun'];
  updateProgress: TaskRunStoreState['updateProgress'];
  completeRun: TaskRunStoreState['completeRun'];
  failRun: TaskRunStoreState['failRun'];
  cancelRun: TaskRunStoreState['cancelRun'];
  addPresenceUser: PresenceStoreState['addUser'];
  removePresenceUser: PresenceStoreState['removeUser'];
}

// --- ADR-0236 (#321): server could not replay the full gap ---
function registerReplayHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const { queryClient, projectIdRef, lastSeqRef } = deps;
...
function registerTaskRunHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const {
    queryClient,
    projectIdRef,
    setRecalculating,
...
// --- Presence + CPM lifecycle events ---
function registerPresenceAndCpmHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const {
    queryClient,
    projectIdRef,
    scheduleInvalidate,
...
// --- Bulk task mutations + product backlog rerank ---
function registerBulkAndBacklogHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const { queryClient, projectIdRef, scheduleInvalidate } = deps;
...
// --- Risk, comment, note, attachment, link, suggestion events ---
function registerCollaborationHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const { queryClient, projectIdRef, scheduleInvalidate } = deps;
...
// --- Project token, custom-field, estimation-poker events ---
function registerTokenAndFieldHandlers(on: OnFn, deps: WsHandlerDeps): void {
...
// --- Sprint, retro-board, scope-injection, review-curation, milestone events ---
function registerSprintHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const { queryClient, projectIdRef, scheduleInvalidate } = deps;
...
function registerMembershipAndBoardHandlers(on: OnFn, deps: WsHandlerDeps): void {
  const { queryClient, projectIdRef, scheduleInvalidate } = deps;
 ...
 * arguments), so the connection effect never inlines the wall of handlers.
 */
function registerEventHandlers(on: OnFn, deps: WsHandlerDeps): void {
  registerReplayHandlers(on, deps);
  registerTaskRunHandlers(on, deps);
  registerPresenceAndCpmHandlers(on, deps);
  registerTaskMutationHandlers(on, deps);
  registerBulkAndBacklogHandlers(on, deps);
  registerCollaborationHandlers(on, deps);
...
interface InvalidateTimerRefs {
  pending: MutableRefObject<Set<InvalidateKey>>;
  timer: MutableRefObject<ReturnType<typeof setTimeout> | null>;
}

/**
 * Build the trailing-debounce `scheduleInvalidate` used to coalesce a burst of
 * invalidations into a single trailing refetch per query key. High-frequency
 * mutation events (task_*, dependency_*, cpm_complete, bulk/reorder,
 * assignment_*, sprint/rollup, board-activity) route through here instead of
 * invalidating immediately, so a sync batch of N row events produces 1 refetch
 * rather than N. Low-frequency, narrowly-scoped invalidations (per-task
 * comments, members, board config, project-level) stay immediate.
 */
function makeScheduleInvalidate(
  queryClient: QueryClient,
  projectIdRef: MutableRefObject<string | null | undefined>,
  refs: InvalidateTimerRefs,
): ScheduleInvalidateFn {
  return (...keys) => {
    for (const key of keys) refs.pending.current.add(key);
    if (refs.timer.current !== null) return;
    refs.timer.current = setTimeout(() => {
      refs.timer.current = null;
      const pending = refs.pending.current;
      refs.pending.current = new Set();
...
interface ProjectSocketConfig {
  projectIdRef: MutableRefObject<string | null | undefined>;
  tokenRef: MutableRefObject<string | null | undefined>;
  // Guards every reconnect/callback so we never open or retry after unmount /
  // a projectId change (the effect re-arms it true and sets it false on cleanup).
  mountedRef: MutableRefObject<boolean>;
  lastSeqRef: MutableRefObject<number>;
  eventHandlers: Record<string, WsHandler>;
}

/**
 * Owns one project's WebSocket connection lifecycle: ticket mint → socket open →
 * message dispatch → exponential-backoff reconnect (1s → 2s → … → 30s) →
 * auth-reject / teardown. Split out of the connection effect so each step is an
 * independently-scored method rather than a deeply-nested inline closure (#2081);
 * the effect just constructs one, `start()`s it, and `stop()`s it on cleanup.
 * `handleMessage` is a bound field so the same reference can be removed on stop.
 */
class ProjectSocketController {
  private ws: WebSocket | null = null;
  private backoffMs = 1_000;
  private retryTimer: ReturnType<typeof setTimeout> | null = null;

  constructor(private readonly cfg: ProjectSocketConfig) {}

  // Parse → ADR-0236 seq-gate → O(1) dispatch for one incoming frame.
  private readonly handleMessage = (event: MessageEvent<string>): void => {
    ...
    if (typeof seq === 'number') {
      if (seq <= this.cfg.lastSeqRef.current) return;
      this.cfg.lastSeqRef.current = seq;
    }

    // O(1) dispatch. An unrecognized event_type has no handler and is a no-op
    // (e.g. task_duration_changed, intentionally unregistered). Guard the
    ...
    const handler = Object.hasOwn(this.cfg.eventHandlers, event_type)
      ? this.cfg.eventHandlers[event_type]
      : undefined;
  ...
  private connect(): void {
    if (!this.cfg.mountedRef.current) return;

    const pid = this.cfg.projectIdRef.current;
    const token = this.cfg.tokenRef.current;
    if (!pid || !token) return;
    ...
    void fetchWsTicket()
      .then((ticket) => {
        ...
        if (!this.cfg.mountedRef.current || this.cfg.projectIdRef.current !== pid) return;
        ...
        this.openSocket(pid, ticket, this.cfg.lastSeqRef.current);
      })
      .catch(() => {
        if (!this.cfg.mountedRef.current) return;
  ...
  private openSocket(pid: string, ticket: string, since: number): void {
    ...
    ws.addEventListener('open', () => {
      this.backoffMs = 1_000; // reset on successful connection
...
export function useProjectWebSocket(projectId: string | null | undefined): void {
  ...
  const pendingInvalidationsRef = useRef<Set<InvalidateKey>>(new Set());
  ...
  useEffect(() => {
    ...
    const scheduleInvalidate = makeScheduleInvalidate(queryClient, projectIdRef, {
      pending: pendingInvalidationsRef,
      timer: invalidateTimerRef,
    });

    // event_type → handler dispatch table. Handlers receive queryClient / refs /
    // store setters through the `deps` bag; grouped event types register one
    // shared handler under every member key. The controller's handleMessage is
    // a flat O(1) lookup into this table (#2081).
    const eventHandlers: Record<string, WsHandler> = {};
    const on: OnFn = (types, handler) => {
      for (const t of typeof types === 'string' ? [types] : types) eventHandlers[t] = handler;
    };

    registerEventHandlers(on, {
      queryClient,
      projectIdRef,
      seenTaskVersionsRef,
      lastSeqRef,
    ...
    const controller = new ProjectSocketController({
      projectIdRef,
      tokenRef,
      mountedRef,
      lastSeqRef,
