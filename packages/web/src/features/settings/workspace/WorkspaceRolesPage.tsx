...
const ROLE_DESCRIPTIONS: Record<Role, { count: number; hint: string }> = {
  Viewer:    { count: 18, hint: 'Read-only across projects they\'re invited to. Use for execs, auditors.' },
      ...
      <div className="px-6 py-4">
        <div className="grid gap-2.5" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
          {ROLES.map((role) => {
            ...
            return (
              <div key={role} className="rounded-md border border-neutral-border bg-neutral-surface-raised p-3 flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold ${bg} ${text}`}>
                    {role}
                  </span>
                  <span className="tppm-mono text-[11px] text-neutral-text-secondary">{count} people</span>
                </div>
      ...
      <div className="px-6 pb-8">
        <div className="rounded-lg border border-neutral-border overflow-hidden">
          ...
          >
            ...
            {ROLES.map((r) => (
              <span key={r} className="text-[12px] font-semibold text-neutral-text-primary text-center">
                {r}
              </span>
            ))}
          </div>
              ...
              {sec.capabilities.map((cap, ci) => (
                ...
                >
                  ...
                  {cap.grants.map((granted, i) => (
                    <span key={i} className="flex justify-center" aria-label={granted ? 'Granted' : 'Not granted'}>
                      {granted ? (
