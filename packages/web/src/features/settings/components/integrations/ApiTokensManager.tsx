 ...
 * `claude_desktop_config.json` snippet (issue 1481). The snippet's shape must
 * stay in step with `docs/administration/mcp-server.md` and the `trueppm-mcp`
 * package — the command is `trueppm-mcp`, the env vars are `TRUEPPM_API_URL`
 * (the instance origin; the server appends `/api/v1`) and `TRUEPPM_API_TOKEN`.
 */
function McpConnectPanel({ token, onClose }: { token: string; onClose: () => void }) {
