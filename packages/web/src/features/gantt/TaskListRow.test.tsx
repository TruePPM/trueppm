...
describe('TaskListRow', () => {
  ...
  it('renders task name', () => {
    renderWithProviders(<TaskListRow task={base} level={2} />);
    expect(screen.getByText('Design Phase')).toBeInTheDocument();
  });

  it('renders duration and progress', () => {
    renderWithProviders(<TaskListRow task={base} level={1} />);
  ...
  it('critical task has aria-label mentioning critical path', () => {
    renderWithProviders(<TaskListRow task={{ ...base, isCritical: true }} level={1} />);
    expect(screen.getByLabelText(/critical path/i)).toBeInTheDocument();
  });

  it('milestone shows diamond and hides duration/progress', () => {
    renderWithProviders(<TaskListRow task={{ ...base, isMilestone: true, duration: 0, progress: 0 }} level={1} />);
