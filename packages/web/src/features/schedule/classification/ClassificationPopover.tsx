...
export interface ClassificationPopoverProps {
  ...
  tasks: Task[];
  isPending: boolean;
  /** Inline error from a failed cascade; switches Apply to "Retry". */
  error: string | null;
  onApply: (spec: {
    subtree: string;
    cascade: boolean;
...
export function ClassificationPopover({
  ...
  return createPortal(
    ...
    >
      ...
      <div className="px-4 py-3 border-t border-neutral-border flex items-start gap-3">
        <div
          role="status"
          aria-live="polite"
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {error ? 'Retry' : cascade ? 'Apply to subtree' : 'Apply to task'}
          </Button>
        </div>
      </div>
    </div>,
