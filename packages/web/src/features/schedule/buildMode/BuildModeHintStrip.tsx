 ...
 * a `? All shortcuts` affordance to open the cheatsheet. Always visible while
 * build-mode is on; not dismissible in v1 (it is the discoverability surface
 * — dismissing it would hide the only signal that build-mode keyboard shortcuts
...
export function BuildModeHintStrip({ mode, onShowCheatsheet }: BuildModeHintStripProps) {
  ...
  return (
    <div
      role="status"
      aria-label={`Build mode active. Focus state: ${mode}.`}
