...
async function gotoSchedule(page: import('@playwright/test').Page) {
  ...
  // Cross-team "current sprint" lens (#1594) — read by the health popover sprint
  // row and the ⌘K action (relocated in #1680), so every routed page calls this.
  // useMyActiveSprints expects a raw array; without this mock it falls through to
  // the paginated catch-all object above, which crashes the app when the hook
  // iterates it.
