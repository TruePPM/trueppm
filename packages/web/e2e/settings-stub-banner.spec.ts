...
test.describe('Settings stub banner (#538)', () => {
  ...
  test('per-issue dismissal: navigating to another stub keeps the new banner visible', async ({ page }) => {
    ...
    // Workflow is a separate stub page with a different issue ref — its banner
    // must remain visible because dismissal is keyed per pageIssue.
    await page.getByRole('link', { name: 'Workflow & fields' }).click();
