...
const FIXTURE_TASKS = [
  {
    id: 'mt-1', wbs_path: '1', name: 'Implementation Phase',
    early_start: '2026-01-05', early_finish: '2026-02-14',
    duration: 30, percent_complete: 30, is_critical: false,
  ...
  {
    id: 'mt-3', wbs_path: '1.2', name: 'Bob Build',
...
const PROJECT_RESOURCES_WITH_ME = [
  {
    id: 'pr-1', project: FIXTURE_PROJECT_ID, resource: 'r1',
    resource_detail: {
      id: 'r1', name: 'Alice (you)', email: 'e2e@example.com',
  ...
  {
    id: 'pr-2', project: FIXTURE_PROJECT_ID, resource: 'r2',
    resource_detail: {
...
test.describe('Board My tasks filter (#198)', () => {
  test('pill renders and toggling hides non-assigned tasks; chip persists across reload', async ({ page }) => {
    await setup(page);
    ...
    const pill = page.getByRole('button', { name: 'My tasks' });
  ...
  test('"Show all →" chip disables the filter and restores all tasks', async ({ page }) => {
    ...
    await page.goto(ROUTE);
    {
      const _pill = page.getByRole('button', { name: 'My tasks' });
  ...
  test('empty state renders when contributor has no matching tasks', async ({ page }) => {
    ...
    const noMatchPool = [
      {
        id: 'pr-only', project: FIXTURE_PROJECT_ID, resource: 'r2',
        resource_detail: {
          id: 'r2', name: 'Bob', email: 'bob@example.com',
          job_role: '', max_units: '1.00', calendar: null, skills: [],
          is_me: false,
