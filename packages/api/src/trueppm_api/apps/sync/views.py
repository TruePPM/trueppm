...
class ProjectSyncView(IdempotencyMixin, APIView):
    ...
    next ``since`` and the delta is complete. See the comment at the watermark read
    for why the ordering, and not a transaction isolation level, is what guarantees
    this.
