...
def _parse_import_bytes(file_content: bytes, ext: str) -> ProjectData:
    """Parse ``.mpp``/``.xml`` bytes into the importer's project-data shape.

    Defined above ``import_msproject`` rather than inside it so the ``@shared_task``
    decorator below binds to the task and nothing else.
    """
    if ext == "mpp":
...
def _reject_infeasible_graph(
    project_data: ProjectData, filename: str, import_request_id: str | None
) -> None:
    """Raise if the file's dependency graph is infeasible, before any write (ADR-0259).

    A hand-maintained .mpp/.xml routinely carries a cyclic or self-referential
    predecessor link, and the importer persists links with ``bulk_create`` — which
    bypasses ``DependencySerializer``'s per-edge cycle check, the only other gate.
    Without this the file writes an infeasible network that then crashes the CPM
    engine on the next recalc: the exact bug ADR-0259 was written for.

    Validated in the file's own uid space so a bad file is rejected while the project
    is still empty rather than rolled back. The caller runs this OUTSIDE its
    ``atomic()`` block: raising inside would roll the outbox claim back to
    DISPATCHED, so the orphan drain would re-dispatch a deterministically bad file
    forever (ADR-0092) — the same reason a parse failure marks the row DEAD.
    """
    ...
    try:
        ...
        # ImportRequest has no result_summary column to persist the detail on
        # (unlike CsvImportRequest), so the TaskRunTracker's FAILED message is the
        # only surface that outlives the worker — log the human-readable form for
        # the operator as well (#2714).
        logger.warning(
...
def import_msproject(
    ...
    with _get_tracker(self, project_id, initiated_by_id) as tracker:
        ...
        try:
            project_data = _parse_import_bytes(file_content, ext)
        except Exception:
        ...
        # Runs OUTSIDE the atomic() block below — see the helper's docstring for
        # why raising inside it would re-dispatch a bad file forever.
        tracker.update(15, "Validating dependency graph...")
        _reject_infeasible_graph(project_data, filename, import_request_id)
