...
#
# Supply-only extension point, same contract as team_signal_consent_changed:
# projects only emits; consumers register a receiver in AppConfig.ready(). Since
# #1275 the OSS notifications app consumes it (notifications/receivers.py →
# in-app inbox row for eligible voters, ADR-0104 Amendment B); Enterprise may
# register its own receiver to capture the ratification lifecycle in its immutable
# audit. The signal is already deferred via transaction.on_commit (see
# _emit_proposal_changed), so receivers run post-commit and read committed state.
# Email stays opt-in OFF (Priya's hard-NO is on un-opted email, not the inbox).
