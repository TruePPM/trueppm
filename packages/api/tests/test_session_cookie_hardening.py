...
- the base (and therefore prod) DRF posture authenticates the API with JWT plus
  the owner-scoped Personal Access Token surface (#2547); SessionAuthentication
  is re-added only in dev/test so ``client.force_login()`` can populate
  ``request.user`` without minting a JWT.
"""
...
_OWNER_TOKEN = "trueppm_api.apps.projects.authentication.OwnerScopedApiTokenAuthentication"
...
def test_base_auth_posture_is_jwt_and_owner_token_only() -> None:
    """Base/prod authenticate with JWT + owner-scoped PAT — no session second surface."""
    assert base.REST_FRAMEWORK["DEFAULT_AUTHENTICATION_CLASSES"] == [_OWNER_TOKEN, _JWT]
...
def test_dev_test_settings_readd_session_auth_for_force_login() -> None:
    ...
    assert _OWNER_TOKEN in classes
