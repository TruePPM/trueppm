...
| `probes.worker.enabled` / `probes.beat.enabled` | `true` | Master switch for the `celery inspect ping` exec probes on the worker/beat tiers. |
| `probes.worker.liveness.*` / `probes.worker.readiness.*` | 60/60/10/5 and 15/60/10/3 | Per-probe `initialDelaySeconds` / `periodSeconds` / `timeoutSeconds` / `failureThreshold` (plus an inheriting `enabled`). The two are tuned apart because their costs differ: a readiness failure is free (a Celery worker sits behind no Service), while a liveness failure kills the container and can spend the worker's 300s `terminationGracePeriodSeconds` — so liveness detection is derived to match that grace (`failureThreshold x periodSeconds` = 300s) and readiness is tuned for an early first success (#3230). |
| `probes.beat.liveness.*` | 30/60/10/5 | Same knobs for beat, which renders a liveness probe only — it is a pinned singleton behind no Service, so there is deliberately no `probes.beat.readiness`. |
| `probes.worker.*` / `probes.beat.*` flat keys | `null` | `initialDelaySeconds`, `periodSeconds`, `timeoutSeconds`, `failureThreshold`, `kubeletTimeoutSeconds` set directly on the component are a **shared override**: one value applied to both probes, taking precedence over the per-probe block. That is what they meant before the split, so an existing values file renders unchanged. |
| `probes.worker.kubeletTimeoutSeconds` / `probes.beat.kubeletTimeoutSeconds` | `null` | kubelet's own probe timeout. Empty derives `max(timeoutSeconds + 5, 1.5 x timeoutSeconds)` — it must cover Celery's ping budget plus the `sh` fork, Python start, and Django/Celery import each exec probe pays. If kubelet fires first there is no Celery stderr, so the failure reads as a bare `Liveness probe failed:` with an empty body. |
