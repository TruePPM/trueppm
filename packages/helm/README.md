...
| `image.tag` | `""` | Empty resolves to `v<appVersion>` (e.g. `v0.4.0-beta.3`) — the tag form on the default GitLab registry. GHCR (`ghcr.io/trueppm/{api,web}`) publishes the **bare** version (`0.4.0-beta.3`, no `v`), so set this explicitly when you repoint `image.repository` there. A value set here is used verbatim. |
...
| `probes.api.hostHeader` | `""` → ingress host, else api Service name | `Host` header kubelet sends on both api probes. kubelet dials by pod IP, so without it Django validates `<podIP>:8000` against `ALLOWED_HOSTS` and answers 400 — the pod never turns Ready and the Ingress serves 503. Empty resolves to `ingress.hosts[0].host` when `ingress.enabled: true`, otherwise to the api Service name `<release>-trueppm-api` — the name `helm test` already curls, so a no-Ingress install needs no value here. Whatever it resolves to must be in `ALLOWED_HOSTS` (#3237). |
| `probes.worker.enabled` / `probes.beat.enabled` | `true` | Master switch for that component's probes. |
| `probes.worker.heartbeatFile` | `/tmp/trueppm-celery-worker-heartbeat` | File a Celery signal handler touches on `worker_ready`/`heartbeat_sent` and removes on `worker_shutting_down` (#3346). Both `probes.worker.startup` and `probes.worker.readiness` stat this same path. |
| `probes.worker.startup.*` | `0`/`5`/`5`/`30` | Checks the heartbeat file **exists** — true once the worker has fired `worker_ready` at least once, i.e. its broker connection is established. Suspends liveness until it passes. |
| `probes.worker.liveness.*` | 60/60/10/5 | Still `celery inspect ping` against the pod's own worker node — a missed check only costs a bounded container restart, so it keeps the exec-probe mechanism. Detection is derived to match the worker's 300s `terminationGracePeriodSeconds` (`failureThreshold x periodSeconds` = 300s). |
| `probes.worker.readiness.*` | 15/15/5/3, `staleSeconds: 30` | **Not `celery inspect ping`** (#3236, #3346) — checks the heartbeat file's freshness. The file is refreshed by Celery's own `heartbeat_sent` signal on a fixed ~2s timer independent of task load, so — unlike the pre-0.4 ping-based check — a busy-but-healthy worker cannot fail this probe. Readiness has no failure budget (one success is all it needs), which is why the mechanism changed rather than the timing. |
| `probes.beat.liveness.*` | 30/60/10/5 | Same `celery inspect ping` mechanism for beat, which renders a liveness probe only — it is a pinned singleton behind no Service, so there is deliberately no `probes.beat.readiness`. |
| `probes.worker.*` / `probes.beat.*` flat keys | `null` | `initialDelaySeconds`, `periodSeconds`, `timeoutSeconds`, `failureThreshold`, `kubeletTimeoutSeconds` set directly on the component are a **shared override** applied across that component's probes, taking precedence over the per-probe block. That is what they meant before the #3230 split, so an existing values file renders unchanged. |
| `probes.worker.kubeletTimeoutSeconds` / `probes.beat.kubeletTimeoutSeconds` | `null` | kubelet's own probe timeout for the `inspect ping`-based probes (worker liveness, beat liveness). Empty derives `max(timeoutSeconds + 5, 1.5 x timeoutSeconds)` — it must cover Celery's ping budget plus the `sh` fork, Python start, and Django/Celery import each exec probe pays. If kubelet fires first there is no Celery stderr, so the failure reads as a bare `Liveness probe failed:` with an empty body. The heartbeat-file probes (worker startup/readiness) have no such derivation — the check is a filesystem stat, so `timeoutSeconds` is used directly. |
| `probes.web.readiness*/liveness*Seconds` | 5/10, 10/30 | Initial-delay and period tuning for the web (nginx) tier's `GET /` probes. No startup key — the container serves a pre-built static bundle with no boot-time dependency to cover. |
