...
Starting with the 0.4 beta, **GHCR is the public release target**, not an optional mirror. The image/chart publish jobs push to GHCR in addition to the internal GitLab container registry, and a missing `GHCR_USER`/`GHCR_TOKEN` now **fails the job** rather than skipping — so a release tag can never appear to succeed without the public artifacts landing. Cosign signing is keyless (Sigstore via the GitLab `SIGSTORE_ID_TOKEN` OIDC audience) and needs no key material. Configure `GHCR_USER` + `GHCR_TOKEN` (Masked + Protected) before pushing the first `v0.4.0-beta*` tag.
...
**CI publish job fails** — the failure is in the build or push itself (Docker build error, registry outage, expired credentials), so read the job log. As of the 0.4 beta a *missing* `GHCR_TOKEN`/`GHCR_USER` **fails** the image/chart publish jobs with an actionable error (GHCR is the public release target, not optional) — set both in GitLab CI/CD variables (Settings → CI/CD → Variables, Masked + Protected) with a PAT that has `write:packages` scope before tagging. The PyPI/npm jobs (`api:publish:pypi`, `web:publish:npm`) still exit 0 when their token is absent.
