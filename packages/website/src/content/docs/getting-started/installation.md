...
Before you put a real program on it, read the **[tested scale envelope](/administration/sizing/#tested-envelope)** — the scale ceilings measured against the 0.4 beta build, which dimensions are still untested, and the issue behind each ceiling. The short version: plan on the Schedule view staying comfortable up to roughly **2,000 tasks** per project (about **1,000** on 0.4.0-beta.1, which does not turn off PostgreSQL's JIT compiler — the envelope explains how to), and on a program's total task count, not just one project's, for schedule recalculation.
