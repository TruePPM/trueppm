...
endpoint you can scrape and alert on.

:::note[Edition]
Routing dead-letter alerts to PagerDuty or Slack is an Enterprise extension (see
[Routing alerts off-box](#routing-alerts-off-box-enterprise)). This page covers the
basic detection layer every adopter gets: the durable record, the alert log line,
...
Enterprise registers against the OSS signal, never the reverse. The signal's shape
is a stable extension contract — OSS does not change it without treating it as a
