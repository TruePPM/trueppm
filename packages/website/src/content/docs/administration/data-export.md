...
:::note[Ships in 0.4 — email withheld from non-workspace-Admin exports]
...
role is self-grantable (any account can create a project and become its
Owner), so on 0.3 that project role alone is enough to pull every resource's
and every member's email address through this endpoint.
:::
