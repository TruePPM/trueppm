...
| `FRONTEND_BASE_URL` | _(empty)_ | Public origin the web app is served from, e.g. `https://trueppm.example.com`. Used to build absolute task deep-links in notification emails (e.g. `task.blocked`). Leave empty to omit the link — emails still carry the blocker type, age, and actor. No trailing slash, no path. |
