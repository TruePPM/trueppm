...
| `TRUEPPM_PUBLIC_BOARD_SHARING_ENABLED` | `true` | Org-wide kill switch for [public board sharing](/features/board-sharing/). When `false`, project Admins cannot mint public board links (`403`) **and** every existing public link stops resolving (`404`) instance-wide — the operator lever for locked-down environments. No board data is exposed until an Admin explicitly creates a link, so the default is on. |
