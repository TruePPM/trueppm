...
|---|---|---|
| `replicaCount` | `1` | API tier replica count. Raise to `2+` for production (the prod overlay sets 2). Request throughput scales with this because uvicorn runs one worker per pod by default — raise `api.workers` below to also scale within a pod. |
...
`image.webRepository` at GHCR, also set `image.tag` to the bare version. A mismatch
surfaces as `ImagePullBackOff` on the `migrate` init container with `manifest
unknown`. A mirror that retags may use either form — set `image.tag` to whatever
yours carries. See [Image pull failure](/administration/troubleshooting/#image-pull-failure).
...
| `probes.worker.heartbeatFile` | `/tmp/trueppm-celery-worker-heartbeat` | File a Celery signal handler touches on `worker_ready`/`heartbeat_sent` and removes on `worker_shutting_down` (added in 0.4, #3346). Both `probes.worker.startup` and `probes.worker.readiness` below stat this same path. |
...
| `env.CSRF_TRUSTED_ORIGINS` | `""` | Comma-separated scheme+host origins Django's CSRF middleware trusts, e.g. `https://trueppm.example.com`. **Required** whenever the web app's origin differs from the API's (a split-origin Ingress such as `app.example.com` + `api.example.com`) — without it, SSO login succeeds and sets the refresh cookie, then the SPA's post-login refresh request 403s with a CSRF failure. Set it to the same origin(s) as `env.TRUEPPM_FRONTEND_BASE_URL`. Harmless to leave empty on this chart's default single-origin Ingress. |
...
| `env.TRUEPPM_NUM_PROXIES` | `"1"` | Trusted reverse-proxy depth for real-client-IP extraction. `1` suits the default `ingress.hosts` (`/api` straight to the API Service); routing through the web tier is two hops and needs `2` — `values-demo.yaml` sets that, since demo mode cannot bypass the web tier. A cloud load balancer or CDN in front of the ingress controller changes the count too (see [Networking](/administration/networking/#a-load-balancer-in-front-of-the-ingress-controller)). Too low collapses the per-IP throttles into one shared bucket; too high lets clients spoof `X-Forwarded-For`. Check the value with `security.client_address` on `/api/v1/health/system/`. |
| `env.TRUEPPM_RATE_LIMIT_ENABLED` | `"true"` | Global API rate-limiting kill switch. Leave `"true"` in production. Disabling also requires `TRUEPPM_RATE_LIMIT_DISABLE_ACK`; for load testing only ([details](/administration/configuration/limits/#disabling-rate-limiting-entirely)). |
...
| `alerts.thresholds.demoReset.staleAfterSeconds` | `93600` | Age (seconds) of the last **successful** reset that fires `TruePPMDemoResetStale`. 26h = one full day plus slack over the default daily schedule, so one missed run is tolerated and the alert does not fire against its own cadence. **Raise it if you lengthen `demo.reset.schedule`.** |
| `alerts.thresholds.demoReset.staleFor` | `30m` | How long `staleAfterSeconds` must stay breached before the alert fires. |
| `alerts.thresholds.demoReset.neverSucceededFor` | `26h` | How long the "no successful reset has ever been recorded" condition must hold before `TruePPMDemoResetNeverSucceeded` fires. Must exceed one full schedule period plus slack — 26h suits the daily default. |
...
| `demo.interactive` | `false` | A **separate** switch from `demo.enabled`, and the two do not imply each other. It arms `DemoReadOnlyMiddleware` on the API pod (`TRUEPPM_DEMO_READ_ONLY=true`), which refuses every unsafe HTTP method under `/api/` except sign-in, token refresh and sign-out — regardless of the caller's role — so a demo may publish a login and still be read-only. `demo.enabled` alone renders exactly what it always did. It also renders a third nginx server block (a method fence over `/api/`, `/admin/` and `/ws/` still `404`), an egress-deny NetworkPolicy for the `api` and `celery-worker` pods, and re-aimed login/account throttles — see [Interactive demo mode](/administration/security/#interactive-demo-mode) for the full posture, including what it does **not** protect against (a hostname gate in front of it, the scheduled reset, and host isolation from your own CI runners). |
...
| `demo.throttle.loginAccountRate` | `"1000/min"` | Only rendered while `demo.interactive` is true, as `TRUEPPM_THROTTLE_LOGIN_ACCOUNT_RATE`, unless `env.TRUEPPM_THROTTLE_LOGIN_ACCOUNT_RATE` is already set (that wins). Raises the per-account login-lockout scope (ordinarily `5/min`) for concurrency rather than credential-stuffing resistance — the credential is published, so there is nothing left for that scope to protect. The per-IP `login` scope stays untouched and is the real limiter. |
| `demo.throttle.userRate` | `"60000/min"` | Only rendered while `demo.interactive` is true, as `TRUEPPM_THROTTLE_USER_RATE`. In that mode it **replaces** `env.TRUEPPM_THROTTLE_USER_RATE`, which the chart otherwise always sets — tune the demo's rate here, not in `env`. Raises DRF's per-account `"user"` throttle scope (ordinarily `1000/min`) so a crowd of visitors sharing the one published account cannot throttle each other's ordinary reads. **This is the only throttle bounding a signed-in visitor's reads** — the per-IP `login`/`anon` scopes stop applying once authenticated — so it is a shared-fate budget, not a per-visitor fairness control; a client that authenticates and issues requests aggressively can still consume the whole thing. The default is set high enough to be effectively lifted: a shared bucket fails every visitor at once when it trips, so it cannot protect anyone, and the real bounds are the api pod's capacity and per-IP limiting at the edge in front of the host. |
| `demo.throttle.maxPersonalAccessTokens` | `10` | Only rendered while `demo.interactive` is true, as `TRUEPPM_MAX_PERSONAL_ACCESS_TOKENS`, unless `env.TRUEPPM_MAX_PERSONAL_ACCESS_TOKENS` is already set. Left at its ordinary default rather than re-aimed: the write fence does not allow-list personal-access-token creation, so this cap is never reached in this mode. Rendered anyway so the decision is explicit. |
...
| `demo.reset.schedule` | `0 8 * * *` | Five-field cron, or an `@hourly`, `@daily`, `@weekly`, `@monthly` or `@yearly` macro, in the CronJob controller's time zone (UTC on managed clusters). Anything else is rejected at render time. Once a day by default (#4151) — the reset's job is mainly re-anchoring the sample's dates (ADR-1197 D9), and the interactive mode's write refusal already leaves little else for it to clean up. While `demo.interactive` is true this value also reaches the api pod as `TRUEPPM_DEMO_RESET_SCHEDULE`, surfaced on `GET /api/v1/edition/` as `demo_reset_schedule` so the shell's demo bar can state "resets daily at HH:MM UTC" from it. Raise the reset alert thresholds below if you lengthen it. |
...
- `demo.interactive` together with `persistence.media.enabled`. This mode must give the
  `api` pod no writable media path at all — defense in depth over the write fence, which
  already refuses the upload. The bundled Atlas sample's attachments are external-link
  references, not uploaded files, so nothing is lost by the restriction.
- `demo.interactive` and `networkPolicy.enabled` together with `postgresql.enabled` or
  `valkey.enabled` set to `false`. The egress-deny policy for this mode allows only DNS
  and the chart's **own** bundled datastore pods — it has no rule for a managed/external
  database, so rendering it there would cut `api` and `celery-worker` off from their own
  data store. Enable the bundled subcharts to use this mode, or set
  `networkPolicy.enabled: false` and restrict egress at the platform layer instead.
