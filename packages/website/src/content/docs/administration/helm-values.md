...
| `env.TRUEPPM_NUM_PROXIES` | `"1"` | Trusted reverse-proxy depth for real-client-IP extraction. `1` suits the default `ingress.hosts` (`/api` straight to the API Service); routing through the web tier is two hops and needs `2` — `values-demo.yaml` sets that, since demo mode cannot bypass the web tier. Too low collapses the per-IP throttles into one shared bucket; too high lets clients spoof `X-Forwarded-For`. |
