...
| `networkPolicy.ingressControllerSelector` | `namespaceSelector` matching `kubernetes.io/metadata.name: ingress-nginx`, empty `podSelector` | The `{namespaceSelector, podSelector}` peer the `api`/`web` default-deny ingress policies admit, alongside each other where the chart's own topology needs it (`web`'s nginx proxies to `api`; the `helm test` probe curls `api` directly). Override for a different controller or namespace (e.g. RKE2's bundled `rke2-ingress-nginx`) — the default assumes an `ingress-nginx`-named namespace, and an unconfigured mismatch silently blackholes ingress traffic rather than merely under-protecting it. `celery-worker` and `celery-beat` admit nothing: neither has a Service, so their policy is default-deny with no exceptions. Egress is untouched by any of this — see the caveat above `networkPolicy.enabled`. |
