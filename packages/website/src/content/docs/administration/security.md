...
and the `trueppm-scheduler` / `trueppm-mcp` PyPI packages — is scanned, signed,
and inventoried in the release pipeline before it is published. This has been
live since the 0.4 beta (GHCR publishing, #939); nothing below is planned or
partial.
...
Every image, the Helm chart, and both PyPI packages are signed **keyless** —
Sigstore, via GitLab's own ambient OIDC identity, no key material to manage or
leak. Verify what you pull actually came from this pipeline:
...
The `trueppm-scheduler` and `trueppm-mcp` wheels and sdists carry
[PEP 740](https://peps.python.org/pep-0740/) provenance attestations instead —
visible under "View details" on each release's PyPI page — rather than a
Cosign signature, since they publish through PyPI's own Trusted Publishing
attestation flow.

### SBOM (Software Bill of Materials)

Every published artifact carries a CycloneDX SBOM:
...
| `trueppm-scheduler` / `trueppm-mcp` | [cyclonedx-py](https://github.com/CycloneDX/cyclonedx-python), against the locked `uv.lock` runtime closure | A non-expiring CI job artifact on the release pipeline (`packages/scheduler/dist/sbom.cdx.json`, `packages/mcp/dist/sbom.cdx.json`) — not yet attached to the PyPI upload itself or the GitLab Release; find it on the `scheduler-v<version>` / `mcp-v<version>` tag's pipeline |
