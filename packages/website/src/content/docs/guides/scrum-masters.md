...
2. **Sign in as the person running the sprint.** Open `http://localhost:5173` and sign in as **`atlas-jordan`** — Jordan Blake, Atlas's Product Owner, who works Platform Core's active sprint from a **Team Member** seat there (creating, activating, and closing a sprint is open to Team Members). Atlas seeds no dedicated Scrum Master persona; the agile-only **Aurora** sample does (`aurora-sam`), and the evaluation guide walks it.
