...
2. **Sign in.** Open `http://localhost:5173` and sign in as **`atlas-jordan`** — Jordan Blake, the Product Owner persona: a **Team Member** on Platform Core, where the backlog and sprints live, and **Project Admin** on GTM Readiness. That surface shows the backlog and board the way you work them.
...
**Further out:** a **read-only MCP server** landed in 0.4 — point an AI assistant at your instance to query the backlog, forecast, and sprint status (computed server-side, self-hosted). PO write workflows via assistant (backlog refinement, story sizing suggestions) follow in 0.6.
...
To see the same data from the schedule side, sign in as `atlas-sam` (Sam Okafor, Project Scheduler, **Resource Manager** role) and open **Plan → Schedule** — your stories are child tasks of Sam's work packages, in one hierarchy.
