...
- **Get your data in — the Jira migration** *(returned from 0.5)* (#627) — a one-time Jira migration import, for the team that has decided to move rather than coexist (the continuous read-only Jira sync ships at 0.4). Switching tools is not a persona — it is the funnel stage every persona passes through. It sits here, in the release built around getting data in and out, rather than inside the staffing charter
