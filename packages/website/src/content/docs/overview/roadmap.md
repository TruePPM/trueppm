...
- **Security hardening** (#1717 #2246 #2247 #2248 #3456 #3457 #3598 #3581 #3510 #3511 #3436 #3438) — per-account login lockout layered over the IP-only throttle (which a distributed credential-stuffing attempt walks straight past), a dedicated JWT signing key separate from `SECRET_KEY` with a global session-invalidation lever, a genuine session-only sign-in behind "Remember me", and explicit auth-cookie `SameSite`/`HttpOnly` pinning. Two more endpoints get the scoped throttle their siblings already had instead of falling through to the unscoped per-user default: invite-resend, closing the unenforced email-bomb bound (#3598), and the project burn read (#3581). It also closes several more places a **revoked** member is still treated as present: the weekly program-health digest keeps emailing them program names, health bands, and drill-through links (#3456), the seed exporter writes them into the export roster so an export → import round trip silently restores their access (#3457), the teams roster still counts them (#3511), and their notification list keeps naming projects and tasks with only the snippet redacted (#3510). Reviving a previously revoked membership no longer erases the revocation record it should retain (#3436), and membership creation now reads the caller's role ceiling inside the same transaction as the write it guards, closing a race the locked `partial_update` path already closed (#3438). Revocation that holds on the write path but leaks on a scheduled email, a roster count, or a revival is not revocation
...
- **Multi-arch images — `linux/amd64` + `linux/arm64`** *(moved up from 0.5)* (#3407) — every published `api` and `web` image, on GHCR and the GitLab Container Registry, is a signed multi-arch manifest from `v0.4.0-beta.4` on. Apple Silicon, AWS Graviton, and Raspberry Pi hosts pull the native image with no emulation and no `platform:`/`nodeSelector` override; Cosign signs and CycloneDX-attests the manifest index itself, so `cosign verify` succeeds regardless of which platform resolves
...
A second pass on 2026-09-26 read the same scope through the questions the platform exists to answer — *what work is to be done, what has been completed, and what is my schedule and financial risk* — because those three are what any team built on top of TruePPM asks of it, whatever its vocabulary for the work. The staffing charter, the status update, plan mode, and the PWA already serve them. The pass changed four items at the margin and added one: the worker profile is now a **non-human resource profile** with agents as its first instance; the i18n decision takes **record-type vocabulary** into scope; onboarding starts from **project templates**; and the **cost model** (#754) moves up from 0.8, because the release answered schedule risk in full and financial risk not at all — nothing between the actuals feed and 0.8's EV-lite put a rate on a resource or a budget on a project. The builder-facing pieces that would follow from that lens — attached record types, a per-workspace manifest, tenancy — are deliberately *not* in this release; they are earned by a real team's use, not designed ahead of it.
...
- **First-run onboarding** *(moved from 0.4)* (#725) — a guided setup rail that walks a fresh install from empty dashboard to a running project with real tasks and a schedule: project creation, first task, team invite, and a live-preview mini-board. The rail starts from a **project template** — a named task list with dependencies, three-point durations, and a calendar — so the first schedule appears in the first session rather than a blank Gantt; a template is also the unit a team hands to the next team doing the same kind of work. The full GA-polish pass happens at 0.9; this is the on-ramp that lets a self-hoster be productive in the first session
...
- **Hybrid human/AI scheduling — the shared seam** *(co-headliner)* (#1836) — the first step toward representing AI agents as first-class *resources*, for software teams whose work now runs across people and agents. A **non-human resource profile** on the resource carries throughput, concurrency, and review capacity, extending the resource-allocation model above; an AI agent is its first instance, but the profile is deliberately not agent-shaped — a machine, a rig, or a crew has the same three numbers, and the seam is designed so it holds them without a second model. This is the engine seam the two headliners share, which is why they land together. The rest of the first cut moves to 0.6, beside the engine depth it belongs with: separating **effort** (work content) from **duration** (elapsed time), so an agent's throughput and round-the-clock availability no longer have to masquerade as an 8-hour human day (#1835), is dual-engine work in both the Python and Rust/WASM engines and is cheaper done alongside sub-day scheduling (#1838) than split across two releases. The **team-owned review-gate read** (#1834) — which surfaces when a team's own review queue, not authoring, has become the bottleneck — follows with it. That queue signal is a team signal by design: team-owned, opt-in to roll upward, never a management surveillance surface ([team ownership is not surveillance](/overview/team-ownership-not-surveillance/))
- **Project status update** *(headliner, added 2026-09-05)* (#3425) — a PM-chosen RAG, a written narrative, and a snapshot of the computed facts captured at post time: milestones with baseline variance, CPM finish vs baseline, P50 / P80, top risks, completed and slipped counts, open decisions. History on the project Overview, every project's latest update on the program Overview (#1596), a one-page PDF (#1200), and a read-only share render over the 0.4 share-link token (#147, moved up from 0.8). The narrative is human-written in this release; the generated draft (#370) stays at 0.8 and drafts from the same snapshot. Single-project status is OSS; report builders, scheduled delivery, and cross-program packs are the Enterprise half, per ADR-1048 (proposed)
...
- **Cost model — the financial half of risk** *(moved up from 0.8, added 2026-09-26)* (#754) — a budget on the project, an effective-dated rate on the resource, and actual cost as time entries × rate. The engine then emits a **P80 cost beside the P80 date**: the Monte Carlo prices each sampled duration at the rate of the resource doing it, in both the Python and Rust/WASM engines under the same conformance gate. This is what turns "you will be three weeks late" into "three weeks late and this much over" — the sentence the status update above reports, the dollar delta plan mode below carries beside its date delta, and the input the program rollup's `cost_variance` and `budget_utilization` KPIs have been declaring without a source. Labor cost only in this release; non-labor direct costs (equipment, cloud, agent spend) follow in #2557, and the full PV/EV/AC surface stays at 0.8 as EV-lite
...
- **i18n framework decision** (#728, moved up from 0.9) — decide string externalization while the UI surface is still small; the self-hosting community is heavily international, and retrofitting extraction gets more expensive every release. The decision takes **record-type vocabulary** into scope alongside language: the nouns for project, task, resource, baseline, and program are a first-class namespace that a workspace can override — a job shop calls a project a *job* and a task an *operation*, a plant calls a program an *outage* — through the same mechanism a translation uses, so the two never need reconciling later. Translations themselves can follow after GA
...
- **MCP plan mode — the agent dry run** *(headliner, pulled forward from 0.6)* (#1813) — the safest slice of the agent write path, shipped a release early because it commits nothing: an agent submits a **proposed** change over MCP with `dry_run` and the engine answers with the verdict, the constraints that fired, and the projected schedule impact — new critical path, date deltas, and, with the cost model above, the cost delta — or a feasibility refusal with its derivation. The same call answers the question every team asks before it says yes to new work — *if we take this on, what happens to everything we have already promised?* — which is why it is the one item in this release that serves the agent story and the human one with a single surface. The *refuse* verb reaches the write side with zero mutation risk, gated by the **same rulebook as human writes** (sprint sovereignty, phase locks, graph validation) — no agent side door. Builds on the 0.5 agent service accounts (#1811), **agent-as-audited-actor** identity scoping (#1063 — extends the 0.4 audit foundation with a capability-scoped actor identity ahead of the write path), and the audit foundation already in `main`; the committing write surface, engine-as-referee enforcement, and standing subscriptions complete the path at 0.6
- **Workshop facilitation core** (#1396 #1397) — a shared timer and dot voting on the Live Retro Board, riding the existing realtime channel with no new trust surface; the full collaborative-canvas shell (Excalidraw, guest links) follows at 0.7 (#1281). Of everything in this release it is the item furthest from the three questions above, so it is the first to move to 0.6 if the window runs short
...
- **Fully cloud native — basic HA in the OSS core** (#3408) — operator-managed in-cluster HA PostgreSQL with automatic failover and WAL archiving (#3403), replicated Valkey with Sentinel and a CI failover drill (#3404), one application Secret contract with an ExternalSecret template and rollout-on-rotation (#3405), and zero-downtime rotation of every application key (#3406). Multi-arch images moved up to 0.4 (#3407). External managed datastores stay fully supported; cross-region and DR remain Enterprise
...
:::note[Native mobile left this roadmap on September 16, 2026]
Until then, this section listed a native Android app as a 0.6 co-headliner and called it "a
hard commitment … it cannot slip again", with iPhone and iPad parity following at 1.0. We have
withdrawn that commitment (#3834). No user of the 0.4 beta has asked for a native app yet: the
case for one rested on personas we modeled, not on reports from people running TruePPM, and
the field-work scenario that motivated offline schedule editing is one we have since judged a
poor fit for this product. The need that remains — logging time in seconds from a phone, and
hearing about a real blocker without being spammed — is what the 0.5 installable PWA and push
notifications are built for. The server-side offline sync protocol and the WASM engine stay, so
a native app remains cheap to start later. It returns to a numbered release when real users
name a workflow the PWA cannot serve.
:::

- **Multi-format import with preview** (epics #624, #613) — top-10 PM tools (Jira, Asana, Monday, Wrike, ClickUp, Planview, Trello, Notion, Linear, Basecamp). CSV/Excel lands earlier still, with the 0.4 beta — 0.6 adds the breadth and the preview polish, and the specialist long-tail (Primavera P6 XER/PMXML, OmniPlan, GanttProject, MPX/ProjectLibre — #630–#635) follows at 0.7 so this release keeps its focus on the write surface and resource leveling
...
- **MCP write surface** (#505 #604) — write tools (create/update task, move card, log time, update status), session auth, and broader surface coverage layered on top of the read-only MCP server that shipped in 0.4, with read restrictions on sprint-internal fields so automation never becomes surveillance
...
- **Change requests, human or agent** *(moved from 0.7)* (#1312) — a proposed scope or schedule change with its projected impact and a disposition by one PM-chosen approver, raised by a PM or by an agent through plan mode. The PM's own change log, which the waterfall-governance audit found missing entirely; multi-approver boards and mandated approval stay Enterprise (ADR-1048, proposed)
- **Issues register** *(moved from 0.8)* (#2318) — the "I" in RAID beside the shipped risk register and decisions: a program-scoped register with a project sub-view. Cross-program transfer may follow later
...
- **OSS integration connectors** — calendar export, meeting links
...
- **Resource leveling — the OSS engine** (#1442) — a resource-constrained pass layered on the CPM result: where parallel activities need the same person, leveling shifts them within their float, then beyond it, until no resource is loaded past capacity. CPM assumes infinite resources, and today TruePPM will happily compute dates that assume one engineer does three critical tasks at once; the 0.5 pre-commit warning tells you that conflict exists, and this is the pass that resolves it. Single project or program, in the OSS core — this is the largest functional gap between TruePPM and the traditional schedulers, and closing it in the free core is the point. Leveling **decision derivation** — per-activity "why did this move?" — lands with it (#2624); resource-constrained Monte Carlo so P80 reflects the leveled plan (#2626) joins this release **only if** the four Monte Carlo correctness fixes it depends on are all closed by the end of 0.5 — a statistical check of the sampler against a known distribution (#3546), CPM dates that move the right way under finish-to-finish and start-to-finish links (#3806), what-if bands that respond for velocity-sampled tasks (#3533), and the risk register feeding the forecast (#3660); otherwise it stays at 0.7. Layering leveling onto a sampler whose percentiles cannot yet be checked would make a wrong P80 indistinguishable from a right one. Rust/WASM parity for offline recompute (#2627) follows at 0.7. Cross-*program* leveling stays enterprise
...
The rule we sequence by: **decisions that keep future options open are made now; the order of
features after 0.6 is set by beta evidence.** In practice that means extensibility and evidence
— extension points, stable API contracts, provenance, the audit record — go into the OSS core
as we build, while Enterprise-specific boundaries and governance wait until their requirements
are concrete. A second filter shapes everything below, per the note at the top of this page:
does this serve the plan a human runs, the grounded facts an agent cites, or both? If neither,
it waits in Unsequenced.
...
PM never maintain two representations of the same work. The **mindmap view** (#2093) that was
sequenced alongside it — the live WBS as an editable node tree — moves to Unsequenced: it is a
real idea, but a second authoring surface for the same plan is exactly the kind of platform bet
that does not need to exist before either the PM's plan or the agent's judgment layer is solid.

**The trust gradient for agents** *(currently 0.7; the change-request object itself moved to 0.6 on 2026-09-05 and accepts a human proposer too)* — **Change Requests** (#1312) route agent
plan-mode proposals — and, since the re-scope, a PM's own scope changes — to a **named approver** who sees the projected schedule impact and records
...
audit chain (#1805) — the *reproduce* half of the AI contract made legible to a human reading a
status report, not just to an agent citing a derivation. A lighter **EV-lite** (#2139) rides
alongside it — PV/EV/AC with SPI/CPI over baselines, the timesheet actuals, and the 0.5 cost
model (#754), all already shipping — with cost reports, a configurable fiscal year, and deeper earned-value surfaces held to
Unsequenced until real programs ask for them. The **Team Cohesion technical preview** (#1488)
also publishes here, as a post and an experimental flag — a real bet, but an independent one:
it does not gate 0.8, 0.9, or 1.0, and if it does not hold up under real teams it stays a
preview rather than becoming the 1.0 marquee, the same "unproven by design" posture the 0.6
falsification spike (#1998) holds itself to. Single-project narrative is OSS; cross-program
schedule forensics stays enterprise.

**The program as one view** *(currently 0.8)* — one timeline across a program's projects,
cross-project dependency lines, program rollup, and risk-slip
propagation, plus an opt-in read-only RAG health digest at the program level. This is also
where the five `program_*` WebSocket events become deliverable (#836). The single-program
**resource leveling** engine this view surfaces is sequenced earlier, at 0.6 (#1442).
Cross-*program* portfolio rollups stay enterprise.
...
AI-Act bundles, retention, legal hold) stays enterprise, building against that seam. 0.9 is
also where we expect to leave the beta line for good — GA hardening is the last `beta.N` cycle
before `1.0.0`, not a promise about which date that lands on.

**1.0 — first stable release** — the marquee is completion, not a single new feature: **human +
agent coexistence substrate** (GitLab epic #1315, labeled `killer-feature`) — a team running
agents safely against one deterministic plan-of-record they plan against, act on, and answer
to, with humans staying the ones who decide. The AI-native arc threaded across this whole
roadmap completes here: the read-only MCP server, provenance, and the agent-action audit
foundation (0.4), the write surface with engine-as-referee and standing subscriptions (0.6),
and the trust-gradient stages above (0.7) land as one coherent substrate rather than a
scattered feature list — judged by the same bar as the human-facing plan it sits beside:
computed, cited, refusable, and reproducible, for a person or an agent equally. Beside it:
workflow-engine maturity (ADR-0080: dead-letter, history API, idempotency hardening,
observability, a second DBOS backend) and a pre-1.0 sample-project refresh. The installable PWA
is the mobile surface at 1.0; the iPhone and iPad parity once planned here left the roadmap
with the native app (#3834). **Team Cohesion** (epic #582) — a Brooks'-Law friction coefficient
that feeds Monte Carlo — remains the most differentiated single idea on this page, but it does
not gate 1.0: it ships as its own release once the 0.8 technical preview validates against real
teams, the same "unproven by design" posture the falsification spike holds itself to. Org-scale
governance of the coexistence substrate — immutable cross-team audit, mandatory approval
policy, capability roles, model/data-egress governance — is the enterprise overlay.

**Unsequenced** — intended, but not placed against any release, and deliberately so: none of
these earn their place against the filter at the top of this page until real usage — ours
building TruePPM, or a self-hoster's — asks for them specifically. The **mindmap view**
(#2093), the **collaborative canvas shell** (#1281 — Excalidraw-based, CRDT co-editing, guest
links), and the public **Extension SDK** (#2094) it was meant to dogfood are the biggest of
these: each is a real, self-contained product, not an incremental feature, and none is required
to run a plan or to give an agent something true to cite. The **importer long-tail** (Primavera
P6 XER/PMXML, OmniPlan, GanttProject, MPX/ProjectLibre — #630–#635) sits here too, behind the
top-10 breadth that ships at 0.6. **Methodology Packs** (epic #577) — versioned phase bundles
that slot into existing projects, with mechanical validation (cycle detection, milestone
reachability, role coverage) and a local pack registry (file / git / http sources) — joins
them. Deeper earned-value surfaces on the Schedule view (EV-lite itself is above), cycle-time
and throughput analytics on the board, and sub-tasks and checklists on stories sit here too.
**Methodology Marketplace** and **Automated Cohesion Inference** are enterprise-edition
extensions built on the pack format and the Team Cohesion model respectively. When one of these
becomes the next thing we are actually building, it moves up into a numbered release.
