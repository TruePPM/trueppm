...
- **Inbound Jira, coexistence-first** (#1418 #1419 #1420 #1421 #1422 — the web + API slice of umbrella epic #1394, which stays open until its mobile connect leg, #1423, lands at 1.0) — a one-way, personal, read-only Jira → TruePPM card pull into My Work, **refreshed on demand rather than continuously**: connect your own Jira account (Cloud or Data Center / Server) once, and each pull brings your assigned issues in beside your native tasks so contributors never double-enter. The background poll behind it is wired but deliberately dormant at beta — it has no opt-in switch yet, and turning it into a genuinely continuous sync — an opt-in switch on top of the existing poll — is its own tracked feature (#3104). Run TruePPM alongside Jira and get the CPM forecast without asking the team to switch first (distinct from the full one-time migration import, which lands at 0.6). Paired with a minimal computable Jira import (#1664) — an offline **Export → XML** upload from Jira Server / Data Center, one import per file and no live connection, that turns issues into a CPM-schedulable network — and a write-parity + cycle guard (#1665) that validates any agent- or importer-generated task graph before it touches the schedule
