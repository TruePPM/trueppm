...
**Community:** scheduling engine, CPM, Monte Carlo, Schedule (Gantt-style) UI, Board, Sprints workspace, program management (coordinating multiple projects within a program), baseline comparison, offline sync, real-time, 5-role RBAC, REST/WS API, Helm chart, MS Project import/export. Also in the Community edition since 0.4: basic single sign-on (OIDC/OAuth login against your own identity provider), time tracking with a weekly timesheet, and in-app baseline capture. Also in the Community edition: the integrations (Jira import, a personal Jira source, GitLab/GitHub task links, Git-event automation, outbound webhooks), the read-only MCP server, the operational audit log, and read-only share links. Still on the Community roadmap: the installable PWA lands in 0.5 and the native Android app in 0.6.

**Enterprise (separate repo):** portfolio analytics and health scores, cross-program resource leveling, org identity governance (SAML 2.0 federation, SCIM provisioning, LDAP/AD directory sync, enforced org-wide SSO), immutable audit trail, custom roles, approval workflows, the org-wide Jira/GitLab/ServiceNow integration hub, AI scheduling, cross-program and portfolio scenario modeling, multi-tenancy.
