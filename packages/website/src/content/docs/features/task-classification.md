...
:::note[0.3]
The **Classification** controls land in **0.3** (the agile team). The fields are already stored and read everywhere; 0.3 adds the editor. They are purely additive — every existing task keeps its current values (`task` / `flow` / `waterfall`), so nothing changes unless you set them.
:::
