...
**Scope of this audit — it is not a claims review.** It checks: version tense and `documentedFor` declaration (`scripts/check-version-status.sh`), internal links, API route names in `api/reference.md` against the OpenAPI schema, Mermaid rendering, website build, feature-page coverage, and OpenAPI sync (the steps below). It does **not** check edition truth (OSS vs Enterprise claims, #4071), feature existence (that "you can …" resolves to a real route, component, or command, #4072), or command execution (that copy-paste install/upgrade/values blocks run as written, #4073). A green docs audit must not be reported as "docs verified"; the report names the classes checked and lists those three as unchecked. See the matching block in `docs-writer`.

Run in parallel:
1. **docs-writer** *(Sonnet)* — Full documentation audit. Check `packages/website/src/content/docs/features/`, `packages/website/src/content/docs/getting-started/`, `packages/website/src/content/docs/architecture/`, and `packages/website/src/content/docs/administration/` for completeness. Every user-visible feature must have a doc page with correct version callouts and enterprise callouts where applicable. Verify the Docusaurus site (`packages/website/`) builds cleanly and that no internal links are broken. Verify `docs/api/openapi.json` is in sync with the current API surface. Owns website feature/getting-started/administration docs; defer API-reference/schema accuracy to `api-design`.
   - **Version-tense alignment (#807)** — diff every `.md` / `.mdx` under `packages/website/src/content/docs/` against `overview/roadmap.md` (the single source of truth) and against the `SHIPPED` constant in `packages/website/src/content/_release-status.mdx`. Every version mentioned in past/present tense ("shipped in 0.X", "added in 0.X", "In 0.X the Y is …") must be under the roadmap's **## Shipped** section. Any version still under **Underway** or **Planned** must be referenced in future tense ("ships in 0.X", "lands in 0.X", "planned for 0.X"). Run `bash scripts/check-version-status.sh` to catch violations automatically; the `docs:version-accuracy` CI job runs the same gate. Move the version to the roadmap's **## Shipped** section, and bump `_release-status.mdx`, when the release **line reaches the maturity that version promises** — for 0.4, the first `beta` tag; **alpha prereleases on the way to that milestone do not promote it** (#2824). Do not promote merely because a tag carrying the version number was cut: that edit is what licenses `scripts/release.sh` to **delete every `Ships in 0.X` / `Coming in 0.X` callout** in the docs tree (131 blocks across 78 pages for 0.4), and doing it early publishes unshipped features as available with `check-version-status.sh` still passing, because it polices only the reverse direction. `scripts/check-early-promotion.sh` refuses an alpha cut against an already-promoted version whose callouts are still live.
