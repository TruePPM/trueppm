...
- [ ] Confirm `GHCR_USER`/`GHCR_TOKEN` CI/CD variables exist (`glab api "projects/:id/variables" | python3 -c '...'` — masked values won't print, existence will). Required unconditionally on every release tag as of #939 (0.4+); a missing credential fails `api:publish`/`web:publish`/`helm:publish` loudly right after the tag is pushed, not before. See Step 3.
