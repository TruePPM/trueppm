...
- [ ] **Prove what the Helm chart reference resolves to, not just that it packages (#3914).** `ci:helm-publish-dry-run` runs `helm:publish`'s real script on every MR that touches it, so packaging, the version backstop and the `--candidate` registry guard are already exercised; only the credentialed push/sign/attest is unproven. It does not read the registry's current state. Semver ranks a plain `X.Y.Z` above every `X.Y.Z-beta.N`, and Helm skips pre-releases unless asked, so a bare `helm install oci://ghcr.io/trueppm/charts/trueppm` and `--devel` both resolve to a stray plain `X.Y.Z` forever; only deleting that package version fixes it. Before tagging, run `bash scripts/check-chart-registry.sh` (needs network) and confirm it is green, meaning the highest stable chart resolves and every default image tag it would pull exists. Also confirm the registry holds no plain `X.Y.Z` for the pre-release you are about to cut. A red result means a broken chart is live: fix the registry first, because a re-cut cannot displace it.
- [ ] **State the re-cut risk out loud.** For every tag-only job the inventory above leaves "unproven until the tag", say so in the release summary and name the job. A tag pipeline is the first and only run of the job as it now stands, so a version number can be burned. Do not let a clean `/pre-release full` stand in for this. That audit reads the tree, and tag-only jobs run only from the tag commit's CI file.
...
**Once a version is published, there is no re-cut: the next cut is the next version.** PyPI uploads are immutable, so moving a tag under a published version only reds the publish jobs. `release.sh` enforces this via `scripts/release-unpublished-guard.sh`, which refuses when any of the three tags exists on `origin` or when `trueppm-scheduler`, `trueppm-mcp` or `trueppm-api` already has that version on PyPI, and fails closed when it cannot read either. The by-hand tag recreation above is only for a release commit whose tags were **never pushed**.
