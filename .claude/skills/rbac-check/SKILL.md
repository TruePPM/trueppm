...
- [ ] **An instance-conditional field gate skips on CREATE** — a field-level permission check inside `validate()` / `validate_<field>` guarded by `self.instance` (e.g. `if restricted_fields and self.instance is not None:`) only runs on UPDATE; on CREATE `self.instance is None`, so the gate is silently bypassed and any creator can set the restricted field at POST. This is the CREATE-side twin of the PATCH miss above (a Member setting Scheduler+-only fields like `capacity_points` while creating the parent object). Any serializer that restricts *who* may set a field must apply the check on create as well, or pair it with a create-path permission gate. Grep serializer `validate` methods for `self.instance is not None` / `if self.instance` guarding a permission or field-restriction branch.
- [ ] **A client-side route or nav guard must not be coarser than the API permissions it fronts.** The checks above ask whether the server is too permissive; this one asks whether the *client* is too restrictive, which strands capability rather than leaking it. When a diff adds or changes a route guard, a nav-visibility predicate, or any single boolean that admits a user to a screen containing several independently-permissioned sections, enumerate the server predicates behind those sections and compare grain. A page-level guard keyed on one coarse capability flag will silently swallow any section whose own endpoint is gated more permissively — a resource the user owns (rows keyed by `(entity, user)`), or a capability granted by a facet/attribute rather than a role ordinal. The symptom is a user who is redirected away from a screen the server would have let them write on, and it is invisible to API-side testing because the endpoint behaves correctly; only the client denies it. When the grains genuinely differ, either move the finer-grained section out from behind the coarse guard, or make the guard section-aware and render the remainder read-only — do not resolve it by hiding the entry point, which converts a functional lockout into a silent one.
