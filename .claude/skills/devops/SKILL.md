...
Every operator instruction in `packages/website/src/content/docs/administration/` must be executable through **values alone** — that is the Helm contract. An operator who must fork the chart, hand-patch a template, or run a post-renderer to follow our own sizing guide has no supported path.
