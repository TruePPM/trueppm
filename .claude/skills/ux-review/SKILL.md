...
- **Device-reachability of the improvement itself (density/viewport-gated wins)** — a readability or interaction improvement can pass a component-level review at desktop density and still be invisible to the persona it was built for, because the surface pins a *different* density or layout mode on that persona's primary device. The failure is structural: the change is real, but its visible effect is gated to a viewport/density the target device never renders, so a desktop-only review (and a desktop screenshot) shows it working while the primary user never sees it. When a review approves a change to how information is *displayed on a card, row, cell, or badge*, check two things against the surface's primary device: (a) **density/viewport gating** — if the surface forces a fixed density or summary layout on a viewport class (e.g. a compact/summary mode pinned on mobile), does the improvement still manifest there, or is it stranded on a density the primary device never selects? A win that only appears at a density the target persona cannot reach is incomplete, not shipped. (b) **hover-only disclosure with no touch equivalent** — if the meaning the change carries lives in a hover/pointer-only affordance (a `title` tooltip, hover-to-expand, hover-peek) with no tap/press equivalent, it is unreachable on touch; a `title`/`aria-label` satisfies a screen reader but not a sighted touch user, so it is not a substitute for a tap-to-reveal. Identify which device is primary for the surface's audience (a job-site or contributor surface is mobile-primary; an admin or portfolio surface is desktop-primary) and confirm the improvement lands on *that* device, not only on the reviewer's. Severity: a primary-device-stranded improvement on a surface whose lead persona uses the *other* device is **HIGH** (it reads as shipped but isn't, for the person it's for); a secondary surface missing only the touch equivalent is MEDIUM. This is not greppable — it requires knowing the surface's primary device and whether its density and disclosure model reach that device, not just that the change renders at desktop.
