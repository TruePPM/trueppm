...
- Shared types: `packages/web/src/api/types.ts` is **hand-maintained**, not generated (#2609). The `generate:types` script exists but emits a different (paths/components) shape that breaks the build — do not run it blindly. When the API schema changes, update the affected interfaces by hand and check them against `docs/api/openapi.json`. **`types.ts` has a narrow drift gate (#2633)**: `scripts/check-web-types-drift.py` (`lint:web-types-drift`, `make web-types-drift-check`) fails when a field that both an interface and its mapped component schema declare disagrees on primitive kind (number/string/boolean/array/object). It does not check missing/extra fields, nullability, enums or formats, so a green run is not "types.ts matches the server" — see the file's own header. User ids are integer PKs everywhere except `/auth/me/`'s `id` and the comment/mention author payloads, which carry the same PK as a decimal string: compare across them with `isSameUser` (`src/lib/userId.ts`), never `===`
- **The e2e mock layer IS gated, and that is a different artifact from `types.ts` — do not read one as covering the other (#3440).** Every response a Playwright mock fulfills is validated at test time against the response schema `docs/api/openapi.json` declares for that method, path and status; a mismatch throws inside the route handler and fails that spec by name. It is wired in by one line — `installSchemaGuard(page)` at the top of `setupCatchAll` — which is what makes it reach the ~290 specs whose payloads are inlined in the spec body, and `lint:e2e-schema-guard` (also `e2e-schema-guard-check` in `make pre-push`) is what stops that line from being quietly deleted. Four rules fail a spec: an unknown property, a wrong type, an impossible enum, and a null on a non-nullable field. **`missing-required` is measured and deliberately NOT enforced** — a fixture that sends `{id, name}` for a Project is an intentional minimal stand-in. Before "fixing" a failure, decide which artifact is wrong: 5 of the endpoints the guard flags return a bare array while the *schema* wrongly declares pagination (#3649), so correcting the mock there would make it wrong. Operations still unbound live in `e2e/fixtures/schema-guard-waivers.ts` with a reason and an issue each, ratcheted so the list can only shrink; **the guard checks nothing on the 46 operations whose 2xx body is declared free-form `{type: object}` (#3652)** — that is a hole in the schema, not coverage
...
- **Never commit or push directly to `main`** — all changes go through a feature branch and MR, including docs, chores, and hotfixes. The single exception is the release commit: `scripts/release.sh` commits the version bump on `main`, and `/release` pushes it directly (see below)
...
- **Release commits are the one direct push to `main`.** `scripts/release.sh` refuses to run off `main`. It commits `chore(release): bump version to X.Y.Z` there and creates the three tags locally. `/release` then pushes `main`, waits for that commit's `main` pipeline to go green, and only then pushes the tags. Tag-triggered publish jobs refuse to run until that pipeline is green (`tag:wait-for-main`). Only a user invoking `/release` authorizes this push; it is not a pattern for anything else
