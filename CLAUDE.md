...
- **Publisher**: Kelly Hair personally, not MacroDream, LLC, on the published packages (`trueppm-scheduler` and `trueppm-mcp` on PyPI: `authors`, owning account) and in the scheduler's citation metadata (`CITATION.cff`, `.zenodo.json`, `paper.md`). There are no org accounts yet — do not add a company affiliation
...
- Shared types: `packages/web/src/api/types.ts` is **hand-maintained**, not generated (#2609). The `generate:types` script exists but emits a different (paths/components) shape that breaks the build — do not run it blindly. When the API schema changes, update the affected interfaces by hand and check them against `docs/api/openapi.json`. **`types.ts` still has no drift gate (#2633)** — `tsc --strict` over it is evidence that the interfaces agree with their own consumers, not with the server; see the file's own header
