...
The capability check is enforced in OSS by a permission class `HasAgentCapability("<cap>")` that composes with the existing 5-role RBAC — an agent token can never exceed the role of the human who provisioned it (`created_by`), and the per-request capability narrows it further. `schedule:write` is deliberately **absent** from the default agent vocabulary in OSS: an agent may *simulate* (ephemeral, §non-goals) but a write that mutates the durable schedule routes through the same human-gated path as any write. Enterprise's approval workflow (#147) registers as the gate that can *grant* an agent a write capability under policy; OSS ships safe-by-default (read + simulate).

#### 1.3 Team-readable audit hook and event schema

OSS owns a **team-readable** append-only audit of agent actions. "Team-readable" is the OSS/Enterprise dividing line: OSS gives the team a record of what its agents did, visible to project members; Enterprise adds the *immutable, tamper-evident, org-wide* trail on top (#146). OSS does **not** promise immutability or cryptographic tamper-evidence — that is the Enterprise value-add and the thing customers convert for.
...
OSS writes one `AgentActionLog` row per event (a plain `models.Model`, not synced — append-only audit, matching the `SprintBurnSnapshot`/`SprintScopeChange`/`SprintTaskOutcome` precedent in ADR-0111: immutability makes `server_version` unnecessary; the team reads it online via `GET /projects/{id}/agent-actions/`, it does not sync to mobile). It then dispatches `agent_action_recorded.send(...)` **inside the same `transaction.on_commit()`** that commits the underlying write, so the event fires exactly when (and only when) the action durably happened.
...
- **Team-readable audit (OSS) vs immutable audit (Enterprise).** OSS gives the team an append-only, member-visible record. Immutability, tamper-evidence, and the org-wide trail are Enterprise (#146).
- **Engine-as-referee (OSS) vs approval-workflow (Enterprise).** OSS lets the deterministic engine arbitrate (simulate, compute, stamp) and routes agent *writes* through the same human-gated path as any write. The human-in-the-loop *approval workflow* that can grant or gate an agent write under org policy is Enterprise (#147), registered as a gate on the OSS hook.
...
- Scope creep across the OSS/Enterprise line — e.g. adding immutability or scoring to the OSS log. Mitigation: the `ai-review` skill (below) enforces the §3/§4 invariants at design time; `enterprise-check` is the backstop.
- The default agent capability vocabulary could be set too permissively (e.g. shipping `schedule:write` by default), undermining the engine-referee/approval-workflow split. Mitigation: OSS ships read + simulate by default; write capabilities are grantable only through the Enterprise approval gate.
...
- No data migration is implied by this ADR itself — it defines contracts. The implementing OSS issues (#1063, #1065) add their own additive `CreateModel` migrations (`AgentActor`, `AgentActionLog`); pin any new enums (`ActorKind`) via `ENUM_NAME_OVERRIDES` to avoid the `api:schema-drift` "Removed schemas" regression.
...
- **Boundary check:** does the change keep agent identity, the audit hook, and the stamp envelope on the OSS side of §3, and does any audit-immutability / approval-workflow / scoring / persisted-scenario behavior route to Enterprise (§4)? Flags any OSS code that would import `trueppm_enterprise` or add immutability/scoring to the OSS log.
...
6. **Outbox cleanup:** N/A for OSS. `AgentActionLog` rows are **permanent team-readable audit** (like `SprintTaskOutcome`, ADR-0111) — not outbox rows, never purged. Provenance stamps are returned in-response and not stored by OSS at all (Enterprise's archive owns its own retention, #152).
