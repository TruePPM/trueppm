...
6. **Outbox cleanup:** N/A for OSS. `AgentActionLog` rows are **permanent team-readable audit** (like `SprintTaskOutcome`, ADR-0176) — not outbox rows, never purged. Provenance stamps are returned in-response and not stored by OSS at all (Enterprise's archive owns its own retention, #152).
